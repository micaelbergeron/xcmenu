...
static cliparg clipargs[] = {
   ...
   { 0, 'q', "query",      0, arg_query,     NULL,                "\tQuery if xcmenu daemon is running." },
};
...
static int set_clipboard_data(clipdata *c, void *buffer, size_t len) {
   ...
   if (!STRNCMP(s, XCMENU_CMD_SEQ)) {
      len -= strlen(XCMENU_CMD_SEQ);
      for (i = 0, s += strlen(XCMENU_CMD_SEQ); i != LENGTH(cmdseqs); ++i) {
...
static int dmenu_puts(void *calldata, clipdata *c,
   ...
   for (; limit > 0 && (buffer[limit] & 0xc0) == 0x80; --limit);
...
static void handle_notify(xcb_selection_notify_event_t *e) {
   ...
   if (reply->type == atoms[ATOM] ||
      ...
      if (e->target != atoms[UTF8_STRING] || (e->target == atoms[UTF8_STRING] && !isbinary(string, len))) {
...
FUNC_ARG(arg_query) {
   if (check_lock(1)) {
      ERR("xcmenu daemon is not running.");
      return -1;
   }
   return 1;
}
